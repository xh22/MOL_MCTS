...
import rdkit_molecules as molecules
import numpy as np
...
class Molecule(object):
  ...
  def __init__(self,
    ...
    self.init_mol_fp = get_fingerprint(init_mol)
  ...
  def get_valid_actions(self, state=None, force_rebuild=False):
    ...
    self._valid_actions=list(self._valid_actions)
    #tmp=[[self.init_mol_fp,get_fingerprint(state),get_fingerprint(i)] for i in self._valid_actions]
    #self._valid_actions_fp=[np.array(i).flatten().tolist() for i in tmp]
    self._valid_actions_fp=[get_fingerprint(i) for i in self._valid_actions]
